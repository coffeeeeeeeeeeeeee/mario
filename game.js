...
async function init() {
    const recalculateScales = () => {
        ...
        fontSize = Math.round(16 * resolutionScale);
        tileScale = SPRITE_SCALE * resolutionScale;
    };

    window.addEventListener('resize', recalculateScales);
    recalculateScales();

    const baseResolution = { width: 1366, height: 768 };
    const baseFontSize = 20;
    const tileSize = 16;
    const baseTileScale = 4;
    const basePlayerPos = { x: 300, y: 300 };

    const widthRatio = window.innerWidth / baseResolution.width;
    const heightRatio = window.innerHeight / baseResolution.height;
    const resolutionScale = Math.min(widthRatio, heightRatio, 1);

    const playerPos = {
        x: basePlayerPos.x * resolutionScale,
